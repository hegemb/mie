...
#######################
## AS EXCEL:
write.csv2(table1,file.path(resultPath,paste0("table1-descriptive-statistics.csv")))
write.csv2(naVal, file.path(resultPath, paste0("number-of-NA-values-covariates.csv")))
```
...
## Write to file: 
write.csv2(p.val.table1,file.path(resultPath,paste0("p-values-for-descrip-table1-Cohort-vs-all-cancers.csv")))
...
## Write to file:
write.csv2(tab33,file=file.path(resultPath,paste0("table2-ovarian-and-uterus-univariateCox-results-HR-and-CI.csv")))
...
write.csv2(output,file=file.path(resultPath,paste0("univariate-cox-for-forest-plot-HR-and-CI.csv")),row.names=F)


```

```{r test for heterogenity,echo=F,eval=TRUE}
...
## Write to file:
write.csv2(all.pval, file = file.path(resultPath,paste0("table4-pvalues-for-heterogenity-univariate-Cox.csv")))
...
For multivariate Cox analysis, we now only inlude the covariates BMI, smoking status, TML and familiy history of breast cancer.
```{r multivariate Cox, comment=NA}
####################################
## MULTIVARIATE COX 
####################################
source("do_multivariate_cox.R")
#fileName <- file.path(resultPath,"coxfit-objects-uterus-ovarian-of-histological-subgroups.Rdata")
#load(fileName)

## Input is dd1; a data set with lpnr, startAld, stoppAld, event, and covariates,
## and "navn" which is the name of the cancer type (for print output):zx
mult.cox <- function(dd1,navn){
  nn <- names(dd1)  
  tmp.cox <- mapply(function(x,y){oo <- do_multivariate_cox(x); print(paste("Multivariate Cox regression:",y,"-",navn)); print(summary(oo)); return(oo)}, x = dd1, y = nn, SIMPLIFY = FALSE) ## SIMPLIFY = FALSE option will make sure that a list is returned.
  return(tmp.cox)
  }

## List the histological subtypes that we want to investigate:
subVarU <- c("All.endometrial","Endometrioid.clearCell","Serous","All.endometrial.plus.ovary.Endometrioid.clearCell")
subVarO <- c("All.epithelial","Endometrioid.clearCell","Serous","All.epithelial.minus.Endometrioid.clearCell")
```

*** Multivariate Cox regression for uterus cancer

-----------
```{r multi cox uterus, comment=NA}
## Run multivariate Cox regression:
## Remove the columns with parity and duration of OC use:
dd.sub.uterus.multivar <- lapply(dd.sub.uterus,function(x) return(x[,setdiff(names(x),c("parity","totalppdur"))]))
## Do multivariate Cox regression:
uterus.multi.cox.BfPar <- mult.cox(dd.sub.uterus.multivar,"uterus_BfPar")
```


```{r table output multivariate uterus, comment=NA}
## Make table of results. 
## NB!! This table will be imported and used to create the forest plot. 

tableHRCIpval <- function(multi.cox,name){
  hr <- lapply(multi.cox,function(x)round(summary(x)$conf.int[,c(1,3,4)],2)) ## Extract HR, lower.ci and upper.ci.
  nEvent <- sapply(multi.cox, function(x)summary(x)$nevent)
  test <- mapply(function(x,y){t(cbind(x,summary(y)$coef[,5]))}, x=hr, y=multi.cox) # Add p-value
  test <- rbind(nEvent,test)# Add number of cases
  tabell1 <- t(test)
  s1 <- c("HR","LCI","UCI","pval")
  s2 <- rownames(hr[[1]])
  rNames <- sapply(s2,function(x)paste(x,s1,sep="."))
  colnames(tabell1) <- c("nEvent",as.vector(rNames))
  
  ## Invert TML: 
  tmp <- round(1/tabell1[,grep("totalMensLifeBfPar",colnames(tabell1))],2)
  tmp <- tmp[,1:3] ## Keep only the first three rows, as the p-values should not be inverted.
  tmp
  tmp <- tmp[,c(1,3,2)]## Change upper and lower confidence limits:
  colnames(tmp)[c(2,3)] <- colnames(tmp)[c(3,2)] ## Switch name of upper and lower CI.
  tmp
  tabell1[,grep("totalMensLifeBfPar",colnames(tabell1))[1:3]] <- tmp ## Insert the inverted TML into tabell1
  print(round(tabell1,3))
  
  ## Write table to file:
  write.csv2(round(tabell1,3), file = file.path(resultPath,paste0("table-of-multivariate-Cox-regression-",name,".csv")))
  
  ## Make table without CI (to make it smaller):
  oo <- grep("CI", colnames(tabell1))
  tabell1.2 <- tabell1[,-oo]
  }

## Will only show the histological subtypes "All.endometrial", "Endometroid.clearCell", and "Serous" in the plot:
tableHRCIpval(uterus.multi.cox.BfPar[subVarU], "uterus-parity-breastFeed")
```

*** Multivariate Cox regression for ovarian cancer
-----------

```{r multi cox ovarian , comment=NA}
## Run multivariate Cox regression:
## Remove the column with parity and duration of OC use:
dd.sub.ovarian.multivar <- lapply(dd.sub.ovarian,function(x) return(x[,setdiff(names(x),c("parity","totalppdur"))]))
## Do multivariate Cox regression:
ovarian.multi.cox.BfPar <- mult.cox(dd.sub.ovarian.multivar,"ovarian_BfPar")
## Make table of output.
tableHRCIpval(ovarian.multi.cox.BfPar[subVarO], "ovarian-parity-breastFeed")

```


```{r make a table of multivariate results for paper}
tableHR <- function(multi.coxU, multi.coxO,name,sVarU=subVarU,sVarO=subVarO){
  multi.coxU <-  multi.coxU[sVarU]
  multi.coxO <- multi.coxO[sVarO]
  hrU <- lapply(multi.coxU,function(x){mm <- cbind(round(summary(x)$conf.int[,c(1,3,4)],2),summary(x)$coef[,5]); colnames(mm)[4] <- "P-value"; return(mm)}) 
  ## Extract HR, lower.ci and upper.ci.
  hrO <- lapply(multi.coxO,function(x){mm <- cbind(round(summary(x)$conf.int[,c(1,3,4)],2),summary(x)$coef[,5]); colnames(mm)[4] <- "P-value"; return(mm)}) 
  ## Extract HR, lower.ci and upper.ci.
  tt <- c(hrU,hrO)
 write.csv2(tt, file = file.path(resultPath,paste0("table3-multivariate-Cox-regression-",name,".csv")))
 return(tt) 
  }

tableHR(multi.coxU = uterus.multi.cox.BfPar, multi.coxO = ovarian.multi.cox.BfPar, name="uterus-and-ovarian")
...
We want to test if the hazard ratio for each of the four variables for subgroups serous (ovarian + uterus) and endometroid (ovarian + uterus) are different. This will be done using a Wald test.
```{r test for heterogenity multivar}
...
## The output is the p-value for the test of heterogenity:
t1 <- mapply(testHetero, uterus.multi.cox.BfPar[subVarU], ovarian.multi.cox.BfPar[subVarO])
#names(uterus.multi.cox.BfPar)
#names(ovarian.multi.cox.BfPar)

## Print the p-values from the tests for heterogenity:
kable(t(round(t1,2)))
#which(t(t1<.05)

## Write to file:
write.csv2(t(t1), file = file.path(resultPath,paste0("table5-pvalues-for-heterogenity-multivariate-Cox.csv")))
