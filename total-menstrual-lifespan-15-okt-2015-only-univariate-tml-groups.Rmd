...
dd.sub.uterus <- lapply(censIndUterus, function(x) cbind(data[,c("LPNR","innAlderPost","attained_age")],x,data[,c("parity","totalppdur","BMI","royk_group","MOR","totalMensLifeBfPar","tml_groups")]))
...
dd.sub.ovarian <-lapply(censIndOvarian, function(x) cbind(data[,c("LPNR","innAlderPost","attained_age")],x,data[,c("parity", "totalppdur","BMI","royk_group","MOR","totalMensLifeBfPar","tml_groups")]))
```
...
pdf("kaplan-meier-the-way-the-patologist-have-classified-only-events.pdf")
...
pdf("Endo-clearCell-moved-kaplan-meier-only-events.pdf")
...
pdf("serous-moved-kaplan-meier-only-events.pdf")
...
pdf("KM-alder-fra-diagnose-til-død.pdf")
...
```{r tabell 1, echo=F, eval=F}
############################
## MAke descriptive table 1
#############################
source("descriptive-table-artikkel.R")
## Hormonal/reporductive variables (cont)
hc <- c("MENSALD","klimAld","parity","totalamm","totalppdur","BMI","totalMensLifeBfPar")
op <- c("royk_group","MOR")
radNavn <- c("N","Menarche (age)","Menopause (age)","Parity","Cum. breastfeeding (mo.)","Cum. OC use (y)", "BMI","Total menstrual lifesp.","Smoking: ever users (%)","Fam. hist of BC (%)")
...
sub.uterus <- mapply(do_all_cox, dd.sub.uterus, navn=paste(names(dd.sub.uterus),"uterus-grouped",sep="-"),SIMPLIFY = FALSE)
```
...
sub.ovarian <- mapply(do_all_cox, dd.sub.ovarian, navn=paste(names(dd.sub.ovarian),"ovarian-grouped",sep="-"),SIMPLIFY = FALSE)
```

```{r make table of univariate output, echo=F, eval=F}
#########################
...
makeNewTab <- function(navn1, navn2){
  subTabl <- read.csv2(file=file.path(resultPath,paste0(navn1[1],navn2,".csv")))
  for(i in navn1[2:length(navn1)]){
    subTabl <- cbind(subTabl,read.csv2(file=file.path(resultPath,paste0(i,navn2,".csv")))[,4:6])
    }
...
#  subTabl <- subTabl[c("bmi.kont","royk.never","royk.ever","MOR.No","MOR.Yes","klimAldStatus.pre","klimAldStatus.post","paritet.kont","ppdur.kont"),]
 # rbind(subTabl[6:9,],subTabl[1:5,]) #rearrange rows
  }
...
navnFile <- "_created_2015-09-03"
newU <- makeNewTab(navnU,navnFile)
tab3 <- makePri(newU)
tab3

## ovarian
navnO <- paste(names(dd.sub.ovarian),"ovarian-grouped",sep="-")
newO <- makeNewTab(navnO,navnFile)
tab3o <- makePri(newO)
tab3o
