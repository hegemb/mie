...
## Write to file:
write.csv2(tab33,file=file.path(resultPath,paste0("merged-ovarian-and-uterus-univariateCox-results-HR-and-CI",Sys.Date(),".csv")))
...
tableHRCIpval <- function(multi.cox,name){
  ...
  print(round(tabell1.2,3))
  }

## Will only show the histological subtypes "All.endometrial", "Endometroid.clearCell", and "Serous" in the plot:
tableHRCIpval(uterus.multi.cox.BfPar[c("All.endometrial","Endometrioid.clearCell","Serous")], "uterus-parity-breastFeed")
```
...
## Make table of output. We will only show the histological subtypes "All.epithelial","Endometroid.clearCell","Serous"
tableHRCIpval(ovarian.multi.cox.BfPar[c("All.epithelial","Endometrioid.clearCell","Serous")], "ovarian-parity-breastFeed")

```


```{r make a table of multivariate results for paper}
tableHR <- function(multi.coxU, multi.coxO,name){
  multi.coxU <-  multi.coxU[c("All.endometrial","Endometrioid.clearCell" ,"Serous")]
  multi.coxO <- multi.coxO[c("All.epithelial","Endometrioid.clearCell", "Serous")]
  ...
  ## Extract HR, lower.ci and upper.ci.
  tt <- c(hrU,hrO)
 # hr$nEvent <- sapply(multi.cox, function(x)summary(x)$nevent)
   #write.csv2(tt, file = file.path(resultPath,paste0("table-of-multivariate-Cox-regression-",name,Sys.Date(),".csv")))
 write.csv2(tt, file = file.path(resultPath,paste0("For-article-table-of-multivariate-Cox-regression-",name,"-",Sys.Date(),".csv")))
 return(tt) 
  }

tableHR(multi.coxU = uterus.multi.cox.BfPar, multi.coxO = ovarian.multi.cox.BfPar, name="uterus-and-ovarian-parity-breastFeed")
